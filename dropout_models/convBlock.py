...
class Norm2d(nn.Module):
    ...
    def stats(self):
        ...
        return float(torch.mean(self.test_running_mean) / torch.mean(self.train_running_mean)), float(
            torch.mean(self.test_running_var) / torch.mean(self.train_running_var))
...
def norm2d_track_stats(model, is_track):
    model.apply(lambda module: setattr(module, 'track_stats', is_track) if hasattr(module, 'track_stats') else None)
